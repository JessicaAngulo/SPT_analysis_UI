...
if imdif_yes == 1
    ...
    dif_out(:,1) = dif_i(traj_pos<0.5);
    dif_out = dif_out(~isnan(dif_out));
    dif_in(:,1) = dif_i(traj_pos>=0.5);
    dif_in = dif_in(~isnan(dif_in));
end
if mss_yes == 1
    mss_i = app.Analysis.Dif_analysis.By_video.(v).MSS(:,1);
    mss_out(:,1) = mss_i(traj_pos<0.5,1);
    mss_out = mss_out(~isnan(mss_out));
    mss_in(:,1) = mss_i(traj_pos>=0.5,1);
    mss_in = mss_in(~isnan(mss_in));
end
if instv_yes == 1
    outside = logical(outside);
    inside = logical(inside);
    vel_i = app.Analysis.Dif_analysis.By_video.(v).InstantV(:,:);
    vel_out(:,:) = vel_i(outside);
    vel_out = vel_out(~isnan(vel_out));
    vel_in(:,:) = vel_i(inside);
    vel_in = vel_in(~isnan(vel_in));
end
if rad_yes == 1
    rad_i = app.Analysis.Dif_analysis.By_video.(v).Dif_radius;
    rad_in = rad_i(traj_pos>=0.5,1);
    rad_in = rad_in(~isnan(rad_in));
    rad_out = Rad_i(traj_pos<0.5,1);
    rad_out = rad_out(~isnan(rad_out));
end
if theta_yes == 1
...
% hold(h,'on')
% [~,edges] = histcounts(log10(dif_close),'BinMethod','Scott');
% histogram(dif_far,10.^(edges(1,1):0.1:edges(1,end)),'Normalization','probability','Parent',h);
% h.Children(1,1).DisplayName = 'Far';
% histogram(dif_close,10.^(edges(1,1):0.1:edges(1,end)),'Normalization','probability','Parent',h);
% h.Children(1,1).DisplayName = 'Close';
% hold(h,'off')
end